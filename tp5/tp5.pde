...
//Load sounds... requires importing the sound lib in the project
 ...
 startGameMessage1 = alienFonts.getSprite("A = Izquierda");
 startGameMessage2 = alienFonts.getSprite("D = Derecha");
 startGameMessage3 = alienFonts.getSprite("F = Disparar");
 startGameMessage4 = alienFonts.getSprite("E = Empezar");
 
 gameOverMessage = alienFonts.getSprite("La Proxima Trata de Ganar");
 youWonMessage =   alienFonts.getSprite("Ganaste Stark!");
...
void drawStartScreenText(int x, int y){
  ...
  text("Presione E Para Empezar", x, y);
  text("Use A y D para moverte de izquierda a derecha", x, y+30);
  text("Use F para disparar", x, y+60);
}
